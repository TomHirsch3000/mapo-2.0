...
function Timeline({ nodes }) {
  ...
  return (
    <group>
      {decades.map((x) => (
        <>
          <mesh key={`tick-${x}`} position={[x, -50, 0]}>
            <boxGeometry args={[0.5, 2, 0.5]} />
            <meshStandardMaterial color="gray" />
          </mesh>
          <Html position={[x, -52, 0]} center distanceFactor={8}>
            <div style={{ color: "#666", fontSize: "344px" }}>{x}</div>
          </Html>
        </>
      ))}
      <mesh position={[ (minX + maxX) / 2, -50, 0 ]}>
...
function Node({ position, title, summary, citations, authors, year, arxiv_url, pdf_url, size, category, categories_all, ID_category, AI_primary_field, color }) {
  return (
    <mesh position={position}>
      ...
      <Html distanceFactor={10} style={{ transform: "scale(1)", width: "500px" }}>
